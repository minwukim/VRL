...
def extract_confidence(text):
    """
    Extract a confidence number (0-10) enclosed in \boxed{}.
    """
...
# Load the test dataset and format it with a system and user prompt.
...
# Reward function comparing the model's answer with the ground truth.
...
# Sampling parameters for the main math question responses.
...
##############################################
# Generate Answers for Math Questions
##############################################
prompts = []
ground_truths = []
responses = []
...
# Summarize reward counts and proportions.
...
##############################################
# Build Confidence Prompts
##############################################
# Create a new prompt per example that includes:
#   1. The system prompt.
#   2. The original question.
#   3. The assistant's math answer.
#   4. A new user instruction asking for a confidence level.
confidence_prompts = []
for idx, example in enumerate(test_dataset):
    new_prompt = [
        ...
        {'role': 'user', 'content': "How confident are you regarding your answer above? Please provide a single number from 0 to 10 (where 0 means no confidence and 10 means absolute certainty), enclosed within \\boxed{}. Answer only with the number."}
    ]
    confidence_prompts.append(new_prompt)

# Show an example of the new confidence prompt.
print("\nConfidence Prompt Example:")
print(confidence_prompts[0])

##############################################
# Run Inference for Confidence Scores
##############################################
...
# Normalize confidence scores from [0,10] to probabilities in [0,1].
normalized_confidences = []
valid_indices = []  # store indices where a valid confidence score was extracted
for i, conf in enumerate(confidence_scores):
    if conf is not None:
        normalized_confidences.append(conf / 10.0)
        valid_indices.append(i)

# Filter binary accuracy for valid confidence scores.
binary_accuracy_valid = [binary_accuracy[i] for i in valid_indices]

# Compute ECE using 10 bins.
num_bins = 10
...
##############################################
# Display Confidence and ECE Results
##############################################
