...
from GRPO_customed.GRPO_SFT_weighted import GRPO_weighted_SFT_Trainer
...
def reward_correct_a1_agnostic(completions, answer, **kwargs):
    ...
    def check_format_and_correctness(s, gt):
        ...
        # answer format is correct nows
        # look for boxed tag
        ext_string = last_boxed_only_string(match.group(1))
        if ext_string is None: return 0   #No boxed tag found
        
        # if correct, then reward 2
        if verify(parse(ext_string), parse(gt)): return 1
        else: return 0 # extracted but incorrect then reward -0.5
...
trainer = GRPO_weighted_SFT_Trainer(
    model=model_name,
