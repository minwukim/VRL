...
class CustomGRPOTrainer(GRPOTrainer):
    ...
    def _generate_and_score_completions2(
        ...
        device = self.accelerator.device
        prompts = [x["prompt"] for x in inputs]
        print(f"HOW MANY PROMPTS: {len(prompts)}")
        ...
        if self.args.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                print(f"HOW MANY ORDERED SET OF PROMPTS: {len(ordered_set_of_prompts)}")
                print(f"ORDERED SET OF PROMPTS EXAMPLE: {ordered_set_of_prompts[0]}")
        ...
        print(f"HOW MANY COMPLETION IDS: {len(completion_ids)}")
        print(f"COMPLETION IDS EXAMPLE: {completion_ids[0]}")
