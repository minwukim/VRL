...
def reward_func(completions, first_completions, answer, **kwargs):
    """
    For each example:
      - If the first response (A1) has a correct format, return the reward of the second response (A2).
      - Otherwise, return min(reward(A1), reward(A2)), i.e. the worse of the two.
    """

    def check_format_and_correctness(s: str, gt: str) -> float:
        # exactly one </think>, <answer> and </answer>
        if not (s.count("</think>") == 1 and s.count("<answer>") == 1 and s.count("</answer>") == 1):
            return -2
        # extract what's inside <answer>…</answer>
        m = re.search(r".+</think>\s*<answer>(.+)</answer>\s*$", s, re.DOTALL)
        if not m:
            return -2
        # find the boxed content
        ext = last_boxed_only_string(m.group(1))
        if ext is None:
            return -1
        # check correctness
        return 2 if verify(parse(ext), parse(gt)) else -0.5

    rewards = []

    for a2, a1, gt in zip(completions, first_completions, answer):
        r1 = check_format_and_correctness(a1, gt)
        r2 = check_format_and_correctness(a2, gt)
        # if A1 format was OK (r1 not in {-2, -1}), use A2's reward; otherwise the worse of the two
        if r1 not in (-2, -1):
            rewards.append(r2)
        else:
            rewards.append(min(r1, r2))
    return rewards
